#!/usr/bin/env bash
set -euo pipefail
# Invoke private Cloud Run via SA impersonation
...
ID_TOKEN=$(gcloud auth print-identity-token   --impersonate-service-account="$SERVICE_ACCOUNT"   --audiences="$CLOUD_RUN_URL")
